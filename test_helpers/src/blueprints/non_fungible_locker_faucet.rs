...
mod nft_locker_faucet {
  ...
  impl NftLockerFaucet {
    ...
    pub fn mint(&mut self, nft_resource_address: ResourceAddress, nft_resource_amount: IndexMap<ResourceAddress, Decimal>) -> NonFungibleBucket {
      ...
      resource_manager.mint_ruid_non_fungible(LockerNftData { data: nft_resource_amount })
    }

    pub fn get_resources(&self, nft_resource_address: ResourceAddress) -> IndexSet<ResourceAddress> {
      self.nfts.get(&nft_resource_address).unwrap().clone().into_iter().collect()
    }

    pub fn get_resource_amounts(
      &self,
      resource_addresses: IndexMap<ResourceAddress, IndexSet<NonFungibleLocalId>>,
    ) -> IndexMap<ResourceAddress, IndexMap<NonFungibleLocalId, IndexMap<ResourceAddress, Decimal>>> {
      let mut all_res_amount = IndexMap::new();

      for (resource_address, nft_ids) in resource_addresses.iter() {
        ...
        all_res_amount.insert(*resource_address, res_amount);
      }
